...
describe("runtimeCapabilityCanonicalPayload", () => {
  test("matches computeRuntimeHash for same refs", async () => {
    ...
    const fromCanonical = await hashPlainObject(runtimeCapabilityCanonicalPayload(refs));
...
describe("toolSpecCanonicalPayload", () => {
  test("matches hashToolSpecStatic", async () => {
    const t = tool({
      name: "t",
      inputSchema: schema,
      handler: async () => 0,
    ...
    const fromHash = await hashToolSpecStatic(spec);
