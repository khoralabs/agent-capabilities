...
/**
 * Resolves per-tool hashes for a runtime snapshot: static map first, else {@link hashToolSpecStatic}.
...
export async function resolveRuntimeToolRefs(
  ...
  for (const name of sortedNames) {
    ...
    if (h) {
      ...
      if (spec) {
        const base = await hashToolSpecStatic(spec);
...
/**
 * Runtime capabilities hash: enabled tools only, ordered by tool name.
 */
export async function computeRuntimeHash(
  ...
  return hashPlainObject(runtimeCapabilityCanonicalPayload(refs));
}

/**
 * Full pipeline: static hashes → evaluate toolkit → runtime hash + tool refs for persistence.
 */
export async function computeRuntimeCapabilitiesFromEvaluation<
  SP extends { kind: string; name: string },
  Tools extends Record<string, ToolSpec>,
  Env = unknown,
>(
  ...
  const runtimeHash = await hashPlainObject(runtimeCapabilityCanonicalPayload(toolRefs));
  return { runtimeHash, toolRefs, evaluatedTools: tools, nameToStaticHash };
}

/**
 * Hash a {@link ToolSpec} using the same fields as static tool definition (for dynamic-only tools).
 */
export async function hashToolSpecStatic(spec: ToolSpec): Promise<string> {
