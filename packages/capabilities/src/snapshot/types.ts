...
/**
 * Agent static snapshot without composable tree (Smithy `RegisteredAgentWire`).
 */
export type RegisteredAgentWire = {
  agentId: string;
  name: string;
  staticHash: string;
...
/**
 * Runtime slice: capability link + tool refs + affordances + policy closure + toolkit context.
 */
