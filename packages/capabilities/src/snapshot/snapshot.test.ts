...
import { hashToolSpecStatic } from "../hashing/runtime-hashes.js";
...
describe("toolSpecToWire", () => {
  test("hash matches hashToolSpecStatic", async () => {
    const t = tool({
      name: "t",
      inputSchema: schema,
      handler: async () => 0,
    ...
    const hLive = await hashToolSpecStatic(spec);
    expect(hWire).toBe(hLive);
  });

  test("toolCapabilityPayloadFromWire matches toolSpecCanonicalPayload", async () => {
    const t = tool({
      name: "t",
      description: "d",
      inputSchema: schema,
    ...
    if (!spec) throw new Error("expected t");
    const wire = toolSpecToWire(spec);
    const fromWire = await hashPlainObject(toolCapabilityPayloadFromWire(wire));
